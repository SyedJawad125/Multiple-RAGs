...
const PLAN_OPTIONS: { value: SubscriptionPlan; label: string; description: string }[] = [
  { value: 'free',         label: 'Free',         description: 'Up to 50 screenings/month'  },
  { value: 'starter',      label: 'Starter',      description: 'Up to 100 screenings/month' },
  { value: 'professional', label: 'Professional', description: 'Up to 500 screenings/month' },
  { value: 'enterprise',   label: 'Enterprise',   description: 'Unlimited screenings'        },
];
...
interface FormState {
  ...
  monthly_screening_limit: number;
  is_active:               boolean;
}
...
export default function UpdateCompany() {
  ...
  useEffect(() => {
    ...
    const fetchCompany = async () => {
      ...
      try {
        const res = await AxiosInstance.get('/api/user/v1/company/', {
          params: { id: companyId },
        });
        const data = res.data?.data;
        if (!data) { toast.error('Company not found'); router.push('/company'); return; }
        ...
        setFormData({
          ...
          monthly_screening_limit: data.monthly_screening_limit ?? 50,
          is_active:               data.is_active               ?? true,
        });
      } catch (error: any) {
        console.error('Error fetching company:', error);
        const msg = error.response?.data?.detail || error.response?.data?.message || 'Failed to load company';
        toast.error(msg);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await AxiosInstance.patch('/api/user/v1/company/', payload, {
        params:  { id: companyId },
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-black py-16 px-4 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="backdrop-blur-2xl bg-gradient-to-br from-slate-900/90 via-slate-800/80 to-slate-900/90 rounded-3xl border border-amber-400/30 shadow-2xl shadow-amber-500/20 p-10 animate-pulse space-y-6">
            <div className="flex flex-col items-center space-y-4 mb-10">
              <div className="w-16 h-16 bg-amber-400/20 rounded-2xl"></div>
              <div className="h-8 bg-slate-800 rounded w-64"></div>
              <div className="h-4 bg-slate-800 rounded w-48"></div>
            </div>
            <div className="w-24 h-24 bg-slate-800/50 rounded-2xl mx-auto"></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[...Array(6)].map((_, i) => <div key={i} className="h-16 bg-slate-800/50 rounded-xl"></div>)}
            </div>
            <div className="h-24 bg-slate-800/50 rounded-xl"></div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[...Array(4)].map((_, i) => <div key={i} className="h-16 bg-slate-800/50 rounded-xl"></div>)}
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="relative z-10">
            ...
            <div className="text-center mb-10">
              ...
              <p className="text-slate-400 text-lg">
                Editing: <span className="text-amber-300 font-semibold">{companyName}</span>
              </p>
              <p className="text-slate-500 text-sm mt-1">ID: {companyId}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex flex-col items-center space-y-3">
                ...
                <p className="text-xs text-slate-500">
                  {displayLogo ? 'Click to replace logo' : 'Click to upload logo'} (max 2 MB)
                </p>
              </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Company is {formData.is_active ? 'visible and operational' : 'hidden from system'}
                      </p>
                    </div>
              ...
              <div className="flex items-center justify-center space-x-4 pt-4">
                ...
                >
                  ...
                  <div className="relative flex items-center space-x-3">
                    ...
                    <span>{isLoading ? 'Saving…' : 'Update Company'}</span>
                  </div>
                </button>
              </div>
            </form>
