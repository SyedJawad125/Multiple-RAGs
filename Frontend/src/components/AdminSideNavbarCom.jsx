...
import { useRouter, usePathname } from 'next/navigation';
// Added Building2, Briefcase, FileUser, ClipboardCheck for your new buttons
import { 
  Home, Users, FileText, Settings, LogOut, Lock, Eye, Menu, X, Key, 
  ChevronRight, User, Shield, Zap, Globe, Image as ImageIcon,
  Building2, Briefcase, FileUser, ClipboardCheck 
} from 'lucide-react';
...
const AdminSideNavbarCom = () => {
  const router = useRouter();
  const pathname = usePathname();
  const navRef = useRef(null); // Ref for the scrollable navigation area
  ...
  // 1. Update active route & 2. Scroll to active item
  useEffect(() => {
    setActiveRoute(pathname);
    
    // This makes the sidebar "move" to show the active button if it's buried deep in the list
    setTimeout(() => {
        const activeElement = document.getElementById(`nav-item-${pathname}`);
        if (activeElement) {
  ...
  const navigationItems = [
    ...
    },
    {
      id: 'company',
      label: 'Company',
      icon: Building2, // Changed from Shield
      path: '/company',
      roles: ['admin', 'super'],
      color: 'blue'
    },
    {
      id: 'jobs',
      label: 'Jobs',
      icon: Briefcase, // Changed from Shield
      path: '/jobs',
      roles: ['admin', 'super'],
      color: 'amber'
    },
    {
      id: 'resume',
      label: 'Resume',
      icon: FileUser, // Changed from Shield
      path: '/resume',
      roles: ['admin', 'super'],
      color: 'emerald'
    },
    {
      id: 'screening',
      label: 'Screening',
      icon: ClipboardCheck, // Changed from Shield
      path: '/screening',
      roles: ['admin', 'super'],
      color: 'purple'
    },
  ...
  const getUserDisplayName = () => user?.name || user?.username || 'Admin User';
  const getUserRoleDisplay = () => role?.name || user?.role_name || 'Admin';

  const getColorClasses = (color, isActive) => {
    const colors = {
      blue: { active: 'from-blue-600 to-blue-500 shadow-blue-500/30', hover: 'hover:bg-blue-500/10', icon: 'group-hover:text-blue-400' },
      purple: { active: 'from-purple-600 to-purple-500 shadow-purple-500/30', hover: 'hover:bg-purple-500/10', icon: 'group-hover:text-purple-400' },
      red: { active: 'from-red-600 to-red-500 shadow-red-500/30', hover: 'hover:bg-red-500/10', icon: 'group-hover:text-red-400' },
      emerald: { active: 'from-emerald-600 to-emerald-500 shadow-emerald-500/30', hover: 'hover:bg-emerald-500/10', icon: 'group-hover:text-emerald-400' },
      amber: { active: 'from-amber-600 to-amber-500 shadow-amber-500/30', hover: 'hover:bg-amber-500/10', icon: 'group-hover:text-amber-400' },
      cyan: { active: 'from-cyan-600 to-cyan-500 shadow-cyan-500/30', hover: 'hover:bg-cyan-500/10', icon: 'group-hover:text-cyan-400' }
    };
    return colors[color] || colors.blue;
  };
    ...
    <div className="relative h-screen">
      ...
      >
        <div className="relative z-10 h-full flex flex-col">
          ...
          <div className="relative p-6 border-b border-slate-700/50 backdrop-blur-sm">
            <div className="flex items-center justify-between">
              <div className={`flex items-center space-x-3 ${isCollapsed ? 'justify-center w-full' : ''}`}>
                ...
                {!isCollapsed && (
                  <div className="flex flex-col">
                    <h2 className="text-xl font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">BlogCMS</h2>
          ...
          <nav 
            ref={navRef}
            className="flex-1 p-4 space-y-2 overflow-y-auto scrollbar-thin scroll-smooth"
          >
            {navigationItems.map((item) => {
              const Icon = item.icon;
              const isActive = activeRoute === item.path;
              ...
              return (
                <button
                  id={`nav-item-${item.path}`} // ID added for scroll-to-view logic
                  key={item.id}
                  onClick={() => handleNavigation(item.path)}
                  onMouseEnter={() => setHoveredItem(item.id)}
                  onMouseLeave={() => setHoveredItem(null)}
                  className={`w-full flex items-center space-x-3 px-4 py-3.5 rounded-xl transition-all duration-200 group relative ${
                    isActive ? `bg-gradient-to-r ${colorClasses.active} shadow-lg` : `${colorClasses.hover}`
                  } ${isCollapsed ? 'justify-center' : ''}`}
                >
                  <Icon className={`w-5 h-5 relative z-10 ${isActive ? 'text-white' : `text-slate-400 ${colorClasses.icon}`}`} />
                  {!isCollapsed && (
                    <span className={`flex-1 text-left font-medium z-10 ${isActive ? 'text-white' : 'text-slate-300 group-hover:text-white'}`}>
                      {item.label}
                    </span>
                  )}
                </button>
          ...
          {/* Footer actions (User, Password, Logout) */}
          <div className="p-4 border-t border-slate-700/50 space-y-2">
            {!isCollapsed && isAuthenticated && (
                <div className="mb-3 p-4 bg-slate-800/40 rounded-xl border border-slate-700/50">
                    <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-400">{getUserRoleDisplay()}</p>
                        </div>
                    </div>
                </div>
            )}
            
            <button onClick={handleChangePassword} className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl bg-slate-800/40 text-slate-300 hover:text-white">
                <Lock className="w-5 h-5" />
                {!isCollapsed && <span className="text-sm">Change Password</span>}
            </button>

            <button onClick={handleLogout} className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl bg-red-600 text-white">
